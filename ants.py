...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit:
            ...
            #update the exit's entrace to be the palce we're creating
        # END Problem 2
        #self = place we creating
        #exit place that we're connecting to
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        temp = self.place
        while temp != beehive:
            if temp.bees and self.min_range <= distance <= self.max_range:
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.ID = QueenAnt.counter
        print('DEBUG ID: ', self.ID)
        QueenAnt.counter += 1
        print('DEBUG counter: ', QueenAnt.counter)
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        #for i in range(self.ID):
            #print('DEBUG i:', i)
        if self.ID > 0: #selfcounter == 1
            Ant.reduce_armor(self, self.armor)
        else:
            ThrowerAnt.action(self, gamestate)
            # ThrowerAnt.throw_at(ThrowerAnt.nearest_bee(self, gamestate.beehive))
                #self.remove_from(self, place)
            #self.throw_at(self.nearest_bee(gamestate.beehive))
            #ScubaThrower.action(self, gamestate)
        #ThrowerAnt.action(self.game)
        while temp_place != None:
            if temp_place.ant and temp_place.ant.check_double == False and self.ID == 0:
                #if temp_place.ant.check_double == False:
                temp_place.ant.damage *= 2
                print("DEBUG damage:", temp_place.ant.damage)
                temp_place.ant.check_double = True
